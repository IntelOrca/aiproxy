...
function makeConfig(
  ...
  return {
    port: 0,
    backends: [
      { id: "primary", baseUrl: `http://127.0.0.1:${primaryPort}/v1`, weight: 1, priority: 0 },
      { id: "backup", baseUrl: `http://127.0.0.1:${backupPort}/v1`, weight: 1, priority: 1 },
    ],
    sticky: true,
...
Deno.test("rate-limit 429 fails over to backup without marking primary down", async () => {
  ...
  const result = await forwardWithRetry(chatRequest(primary.port), body, "f:abc", manager, config);
...
Deno.test("rate-limit failover re-pins the session to the backend that served", async () => {
  ...
  const config = makeConfig(primary.port, backup.port);
  const manager = new BackendManager(config);
  const body = { model: "gpt-4o", messages: [{ role: "user", content: "turn" }] };
...
Deno.test("rate-limit message in error body triggers failover", async () => {
  const primary = startTestServer(() =>
    Response.json({ error: { message: "you have reached your limit" } }, { status: 400 })
  );
  const backup = startTestServer(() => Response.json({ ok: true }, { status: 200 }));
...
Deno.test("backend apiKey is sent as Bearer to the backend", async () => {
  ...
  const config = {
    port: 0,
    backends: [
      { id: "b", baseUrl: `http://127.0.0.1:${backend.port}/v1`, weight: 1, apiKey: "sekret" },
    ],
    sticky: false,
    healthCheckIntervalMs: 60_000,
    healthCheckTimeoutMs: 1_000,
...
Deno.test("client auth key is stripped upstream when router apiKeys are set", async () => {
  ...
  const config = {
    port: 0,
    backends: [{ id: "b", baseUrl: `http://127.0.0.1:${backend.port}/v1`, weight: 1 }],
    sticky: false,
    healthCheckIntervalMs: 60_000,
    healthCheckTimeoutMs: 1_000,
    sessionTtlMs: 60_000,
